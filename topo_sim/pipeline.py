...
def _topology_scale(name: str, g: nx.Graph) -> str:
    ...
    if name == "DF":
        ...
        return f"{server_count} servers x {exchange_nodes_per_server} exchange nodes/server"
...
def _topology_pattern(name: str, g: nx.Graph, cfg: AnalysisConfig) -> str:
    ...
    if name == "DF":
        return (
            "All Unions participate in one direct Dragon-Fly plane: every server keeps an internal Union full-mesh, "
            f"and each Union also exposes {cfg.df_external_servers_per_union} x 400 Gbps inter-server Dragon-Fly links."
        )
    return (
...
def _topology_configuration(name: str, g: nx.Graph, cfg: AnalysisConfig) -> dict[str, Any]:
    ...
    if name == "DF":
        ...
        backend_ports_per_union = (
            sum(
                1
                for _, _, data in g.edges(sample_union, data=True)
                if data.get("link_kind") == "backend_interconnect"
            )
            if sample_union is not None
            else 0
        )
    else:
        backend_ports_per_union = {
    ...
    if name == "DF":
        topology_cfg["server_count"] = int(g.graph.get("df_server_count", 0))
    ...
    if name == "DF":
        return {
            ...
            "notes": [
                ...
                "servers are built from multiple exchange-node units, and all Union chips inside one server are fully meshed at 400 Gbps",
                "all Union chips across the machine participate in one Dragon-Fly backend plane",
                "each remote server pair uses one unique inter-server Union-to-Union Dragon-Fly link, while source-side and destination-side local segments split across shortest in-server choices",
    ...
    return [
        {
            "title": "A2A",
            "description": (
                f"{total_ssus} SSUs each send {cfg.message_size_mb:.2f} MB to every other SSU."
            ),
        },
        {
...
def _routing_mode_description(mode: str, topology_name: str, cfg: AnalysisConfig) -> str:
    ...
    if mode == "SHORTEST_PATH":
        if topology_name == "DF":
            return "Single shortest-path routing on the Dragon-Fly Union plane."
